      ...
      <div>
        <div className="catalog-controls brushed glass relative overflow-hidden rounded-2xl p-5">
          <div className="relative z-10 flex flex-col gap-4">
            ...
            {/* The active pill is one shared element that slides between chips. */}
              ...
              {chips.map((c) => {
                ...
                return (
                  <button
                    ...
                    aria-pressed={active}
                    className={`relative inline-flex min-h-11 items-center rounded-full border px-3.5 py-1.5 text-xs font-medium uppercase tracking-wide transition-colors ${
                      active
                  ...
                  >
                    {active && (
                      <motion.span
                        layoutId="catalog-chip-active"
                        aria-hidden="true"
                        className="absolute inset-0 rounded-full bg-red shadow-md shadow-red/30"
                        transition={{ type: "spring", stiffness: 400, damping: 34 }}
                      />
                    )}
                    <span className="relative z-10">{c.name}</span>
                    <span className={`relative z-10 ml-1.5 font-mono text-[10px] ${active ? "text-white/70" : "text-text-dim"}`}>
                      {c.count}
                    </span>
                  </button>
                );
            ...
            <AnimatePresence mode="popLayout" initial={false}>
              {filtered.map((p) => (
                <motion.div
                  ...
                  transition={{ layout: { duration: 0.45, ease: EASE }, duration: 0.28, ease: EASE }}
                >
